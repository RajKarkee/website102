...
@section('meta_description', 'Get in touch with B&B Tax for professional accounting services. Located in Auckland, serving businesses across New Zealand.')
...
<div class="min-h-screen bg-background text-foreground">
    ...
    @include('components.jumbotron', [
        ...
        'description' => 'Ready to streamline your accounting? Contact our team of professionals and discover how we can help your business thrive.',
                ...
                <form action="{{ route('contact.store') }}" method="POST" class="space-y-6" data-validate>
                    ...
                    <div class="form-group">
                        ...
                        <select id="service" name="service" class="form-input">
                            ...
                            <option value="payroll-management" {{ old('service') == 'payroll-management' ? 'selected' : '' }}>Payroll Management</option>
                            <option value="accounts-receivable" {{ old('service') == 'accounts-receivable' ? 'selected' : '' }}>Accounts Receivable</option>
                            <option value="accounts-payable" {{ old('service') == 'accounts-payable' ? 'selected' : '' }}>Accounts Payable</option>
                            <option value="gst-filing" {{ old('service') == 'gst-filing' ? 'selected' : '' }}>GST Filing</option>
                            <option value="tax-returns" {{ old('service') == 'tax-returns' ? 'selected' : '' }}>Tax Returns</option>
                            <option value="xero-training" {{ old('service') == 'xero-training' ? 'selected' : '' }}>Xero Training</option>
                ...
                @php
                    $faqs = [
                        [
                            ...
                            'answer' => 'We can typically start managing your payroll within 1-2 business days of receiving all necessary information and documentation.'
                        ],
                        [
                            'question' => 'Do you work with businesses outside Auckland?',
                            'answer' => 'Yes, we work with businesses throughout New Zealand. Most of our services can be provided remotely via secure cloud-based systems.'
                        ],
                        [
                            'question' => 'What accounting software do you support?',
                            'answer' => 'We primarily work with Xero, MYOB, and QuickBooks. We also provide comprehensive training on these platforms.'
                        ],
                        [
                            'question' => 'How do you ensure data security?',
                            'answer' => 'We use bank-level encryption and secure cloud storage. All our systems are regularly updated and comply with New Zealand privacy laws.'
                        ]
                    ];
                @endphp
